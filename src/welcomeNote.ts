...
## ⚠️ Data Safety ⚠️
- There's an automatic backup feature which has various options to allow you to keep a snapshot of the progress of your novel. Backups are saved as zip files inside your vault at \`_sf-backup/\`. (You have an option for daily, weekly, or whenever Obsidian is open.)
- As part of that, there's also a manual backup button which backs up everything within the Obsidian vault, including hidden files like your settings folder.
...
- This leads to one final thing to note: storyForge, other than creating files (including Context panel “Create in Codex” entries for new people/places) and renaming files in the codex pane (and codex pane only), cannot touch the files which are held within the library and codex folders. Fact warnings in Context panel are display-only — you place details into Codex yourself. And storyForge doesn't have any delete function in its code. I set this up like this to make sure your masterpiece is kept as safe as I can make it, especially if you use the automatic backup and follow proper backup procedures (321 backup is a good place to start).
