...
storyForge keeps plugin metadata in `_sf-backstage/`. Manuscript prose under `_sf-storylibrary/` is never modified after a chapter file is created empty. Codex notes under `Codex/` are created and renamed for wikilinks; Story Context may also update the Facts section of a Codex note when you explicitly choose "Update Codex" / "Acknowledge" — that is the only intentional Codex *content* write path.
